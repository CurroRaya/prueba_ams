...
import com.raya.curro.pruebaams.ui.databinding.FragmentDetailInformationBinding
import com.raya.curro.pruebaams.ui.fragments.listfragment.CharacterListFragment
...
class DetailInformationFragment : BaseFragment<FragmentDetailInformationBinding>() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.apply {
            ...
            characterStatus.text = "Estado: ${args.status}"
            characterSpecies.text = "Especie: ${args.species}"
            characterType.text = "Tipo: ${args.type}"
            characterGender.text = "Genero: $gend"
            val dateformatter = SimpleDateFormat("yyyy-MM-dd'T'HH:mm", Locale.ROOT)
            val parsed = dateformatter.parse(args.created)
            val formatted =
                SimpleDateFormat("dd MMMM yyyy", Locale.forLanguageTag("es")).format(parsed)
            var created = "Creado"
            if (gend == "Female") created = "Creada"
